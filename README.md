...
| AI 工作台        | 管理台对话、语音输入、Pi 对话模型用量、权限诊断、系统查询和需确认的管理操作     |
...
AI 助手通过 Pi 记录每次对话模型调用的输入、输出、缓存和总 tokens，并按当前
登录用户汇总本月用量。用量接口为 `GET /api/v1/ai-chat/usage`，只允许读取自己的
记录；删除聊天记录不会删除已经发生的用量事件。金额会尽量按
[models.dev](https://models.dev/) 的 provider、模型和实际 API 地址匹配价目表，并保存调用时的价格快照；私有部署或未匹配到价格时，金额为
不可估算，界面仅显示 tokens，不会把未知价格当成 0。
