...
| 知识库           | 审查后上传/编辑文档、角色权限过滤、向量检索与结构化检索元数据                    |
...
| 会话与可观测性 | 完整保存用户和助手消息、来源文档与待确认操作；LangGraph checkpoint 与内置摘要中间件缩减模型上下文；每轮记录首个 Agent 事件、工具与首个回复 token 的耗时 | 业务消息表始终保存完整历史；checkpoint 仅保存 Agent 运行态，重试生成或删除会话时会清除对应状态，模型或上游连接中断时仍保留已生成的助手内容 |
...
向量列固定为 1024 维，因此部署前需选择兼容的 embedding 模型，例如 Qwen3-Embedding-0.6B-4bit-DWQ。模型、embedding、分块、LangGraph 摘要、超时与 Langfuse 的完整可选配置均在 `apps/backend/.env.example` 中维护；通常只需配置模型连接和 embedding 模型。
...
针对单机部署模型，可执行 `pnpm --dir apps/backend exec node ace ai:evaluate`。该命令使用当前 `AI_OPENAI_*` 配置和 mock 工具，在不连接业务数据库、不创建提案、不执行变更的前提下，验证知识检索、实时系统事实、角色权限、注册查询、缺参追问后的继续查询、受控变更、权限拒绝与自由 SQL 拒绝等场景。每个场景的历史相互隔离，而多轮场景保留自身上下文；每轮都要求调用精确的工具集合，并验证回复是否通过已有的事实依据/范围保护策略。任一失败即返回非零退出码，适合作为模型、量化或提示词变更的回归门禁。Qwen3-4B-Instruct-2507-4bit 建议保持低温度（默认 `0.1`）。
