...
export type SqlTemplateParseResult = {
  variables: SqlTemplateVariable[]
  issues: SqlTemplateIssue[]
}
...
export function extractSqlTemplateVariables(template: string): SqlTemplateParseResult {
  ...
  while (cursor < template.length) {
    ...
    if (unexpectedClose !== -1 && (from === -1 || unexpectedClose < from)) {
      issues.push({
        code: 'unexpected-variable-close',
        from: unexpectedClose,
        to: unexpectedClose + 2,
      })
...
export function extractSqlStructureIssues(sql: string) {
  const issues: Array<{
    code: 'unclosed-parenthesis' | 'unexpected-closing-parenthesis'
    from: number
    to: number
  }> = []
  const stack: number[] = []
  ...
  for (let index = 0; index < sql.length; index += 1) {
    ...
    if (character === ')') {
      ...
      if (opening === undefined)
        issues.push({ code: 'unexpected-closing-parenthesis', from: index, to: index + 1 })
    }
  }
  return [
    ...issues,
    ...stack.map((from) => ({ code: 'unclosed-parenthesis' as const, from, to: from + 1 })),
  ]
}
...
const variableMatcher = new MatchDecorator({
  ...
  decoration: () =>
    Decoration.mark({
      ...
      attributes: {
        style:
          'color: var(--cm-variable); font-weight: bold; background: color-mix(in oklab, var(--cm-variable) 12%, transparent); border-radius: 2px; padding: 0 2px;',
      },
    }),
})
...
export function sqlEditorExtensions(
  ...
  const variableCompletion: CompletionSource = (context: CompletionContext) => {
    ...
    return {
      ...
      options: variables.map((variable) => ({
        label: `{{${variable.name}}}`,
        type: 'variable',
        detail: variable.description ?? 'Variable',
      })),
  ...
  const contextCompletion: CompletionSource = (context: CompletionContext) => {
    ...
    if (
      !lastClause ||
      ['GROUP BY', 'ORDER BY', 'LIMIT', 'FETCH FIRST', 'OFFSET'].includes(lastClause)
    )
      return null
    return {
      from: context.pos,
      options: config.conditionKeywords.map((label, index) => ({
        label,
        type: 'keyword',
        boost: 10 - index,
      })),
  ...
  const sourceCompletion: CompletionSource = (context: CompletionContext) => {
    ...
    for (const match of text.matchAll(
      /(?:\bWITH|,)\s*([A-Za-z_][A-Za-z0-9_]*)(?:\s*\(([^)]*)\))?\s+AS\s*\(/gi
    )) {
      if (match[1])
        columns.set(
          match[1],
    ...
    return {
      from: member.from + source.length + 1,
      options: (columns.get(source) ?? []).map((label) => ({ label, type: 'property' })),
      validFor: /[A-Za-z0-9_]*/,
    }
  }
    ...
    EditorState.languageData.of(() => [
      {
        autocomplete: (context: CompletionContext) =>
          variableCompletion(context) ??
          sourceCompletion(context) ??
          contextCompletion(context) ??
          functionCompletion(context),
      },
    ]),
    keymap.of([
      {
