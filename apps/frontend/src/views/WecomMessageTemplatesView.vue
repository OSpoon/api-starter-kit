...
function openTest(template: WecomMessageTemplate) {
...
async function confirmTest() {
  ...
  try {
    ...
    toast.error(t('wecom_templates.validation.json'))
    return
  }
  ...
  if (missing) {
    toast.error(t('wecom_templates.validation.test_parameter_required', { name: missing.name }))
    return
  }
...
async function sendConfirmedTest() {
  if (!testTemplate.value) return
  testing.value = true
  try {
    await testWecomTemplate(auth.token, testTemplate.value.id, pendingTestParams.value, {
      mentioned_list: pendingMentionedList.value,
      mentioned_mobile_list: pendingMentionedMobileList.value,
    })
    testConfirmOpen.value = false
    toast.success(t('wecom_templates.test_success'))
  } catch (e) {
    toast.error(e instanceof Error ? e.message : t('wecom_templates.test_failed'))
  } finally {
    testing.value = false
  }
}
