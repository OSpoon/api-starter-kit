...
export default defineConfig(({ command, mode }) => {
  const productionExamplesModule =
    command === 'build' && mode === 'production'
      ? './src/router/modules/workbench-examples.production.ts'
      : './src/router/modules/workbench-examples.ts'
  ...
  return {
    ...
    resolve: {
      alias: [
        {
          find: '@/router/modules/workbench-examples',
          replacement: fileURLToPath(new URL(productionExamplesModule, import.meta.url)),
        },
        {
          find: '@',
