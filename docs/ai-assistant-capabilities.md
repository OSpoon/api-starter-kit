...
下表比较管理台默认 AI 助手与三个外部渠道。外部机器人通过统一的 `AiChannelBridge` 接入，
与管理台及独立客户端复用同一套 Agent、知识库、注册查询、受控操作、用户绑定和权限校验；完成外部身份绑定后，
各入口的业务能力原则上保持一致，差异主要来自渠道呈现和运行条件。

| 对比维度         | 默认 AI 助手（管理台）                             | 企业微信机器人                                      | 钉钉机器人                                             | 飞书机器人                                                                                |
| ---------------- | -------------------------------------------------- | --------------------------------------------------- | ------------------------------------------------------ | ----------------------------------------------------------------------------------------- |
| 使用位置         | 登录后的管理台 AI 工作台                           | 企业微信智能机器人                                  | 钉钉企业内部应用机器人                                 | 飞书企业自建应用机器人                                                                    |
| 身份与权限       | 直接使用当前登录用户                               | 私聊绑定系统用户；群聊使用隔离的访客主体            | 同左                                                   | 同左                                                                                      |
| 知识库问答       | 支持，显示完整助手回复和工具状态                   | 支持，回复通过企微消息发送                          | 支持，回复通过钉钉消息发送                             | 支持，回复通过飞书消息或卡片发送                                                          |
| 权限诊断         | 支持 `diagnose_my_access`                          | 仅私聊绑定用户                                      | 仅私聊绑定用户                                         | 仅私聊绑定用户                                                                            |
| 实时系统查询     | 支持注册查询模板，结果最多 20 条                   | 仅私聊绑定用户                                      | 仅私聊绑定用户                                         | 仅私聊绑定用户                                                                            |
| 管理变更         | 支持创建 API Key、用户/角色/权限管理等受控操作提案 | 仅私聊绑定用户，确认卡片确认后执行                  | 仅私聊绑定用户，互动卡片确认后执行                     | 仅私聊绑定用户，Interactive Card 确认后执行                                               |
...
- 群聊访客是例外：不绑定系统账号，只能检索公开知识库；访客会话按群和外部用户隔离，不能访问实时系统数据或发起管理变更。
- 企微、钉钉和飞书的差异不应在业务 service 中分别实现。新增 AI 能力时，应先注册到共享的工具、查询或动作 registry，再由各渠道适配器负责消息和卡片转换。
- 外部渠道更适合快速问答和确认操作；需要完整历史、较长结果、工具过程或连续管理任务时，优先使用管理台或独立助手客户端。查询达到 20 条后，所有入口都应引导用户打开对应管理模块查看完整分页数据。

## 支持范围

| 能力                 | 支持方式                                                                 | 限制                                                                                                                         |
| -------------------- | ------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------- |
| 项目、产品与流程问答 | `search_knowledge_catalog` → `search_knowledge` 两阶段检索               | 必须先检索权限过滤后的知识源目录，再限定 1 至 10 个文档检索正文；只使用正文摘录，目录元数据不是回答证据。                    |
...
- 知识检索每次先校验 `knowledge:read` 并过滤角色范围；目录最多 12 个候选，正文最多选择 10 个目录结果。第二阶段不能使用其他轮次的文档 ID。
- LLM 元数据提取只返回 `summary`（界面显示为“知识源说明”）和 `topics`（界面显示为“可检索主题”）建议，保存前由管理员审核；旧的来源机构、文档类型、产品或业务对象字段已移除。
- 破坏性操作必须通过结构化确认卡，模型文本不是授权通道。
- API Key、密码、token 和密钥值不会进入普通查询结果、聊天历史或日志。

## 配置与观测

模型、Embedding、ASR 和请求超时通过系统管理中的「LLM 配置」页面维护，保存后对下一次请求生效。管理台 Web、独立助手 Web 和桌面端均支持语音输入：录音最长 60 秒、音频最大 10 MB，停止后先由配置的 `Qwen3-ASR-0.6B-4bit`（或自定义模型）转写，再将中文文本交给对话模型。上下文预算由模型的 `contextWindow`、输出上限和 Pi 的 token 估算共同控制；长会话由 Pi compaction 生成滚动摘要。
...
实现细节见 [AI 助手架构](ai-assistant-architecture.md)，知识库细节见 [知识库实现说明](knowledge-base.md)，安全边界见 [安全与治理](security.md)。
